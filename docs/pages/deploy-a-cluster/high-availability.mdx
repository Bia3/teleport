...
- A **public Layer 4 load balancer** to direct traffic from users and services
  to an available `teleport` process.
- ** **private Layer 4 load balancer** to direct traffic from the Proxy Service
...
- **Auth Service load balancer:** A load balancer to forward traffic from a Proxy Service instance to an
  available Auth Service instance. This handles TLS traffic to the Auth
  Service's gRPC endpoint.

Neither load balancer must terminate TLS. Both load balancers must transparently
forward the TCP traffic they receive. In other words, these must be Layer 4 load
balances, not Layer 7 (e.g., HTTP) load balancers.
...
It is also good practice to allow your teams to access Auth Service instances
via Teleport. 

If you are running your Teleport cluster on Kubernetes, you can configure your
load balancer to forward traffic from port `3026` on the Auth Service load
balancer to port `3026` on an Auth Service instance, which you can configure to
run the Teleport Kubernetes Service as well in order to proxy traffic to the
Auth Service pod.

If you are running your Teleport cluster on virtual machines, you can open port
`3022` on the Auth Service and forward traffic to it from the Auth Service load
balancer. You can then run the Teleport SSH Service on your Auth Service hosts
in order to allow secure access.
...
{/*TODO: Edit the config/open ports instructions:

- Separate Auth/Proxy pools
- Run the SSH Service or Kubernetes Service on the Auth/Proxy instances so you
  can access them
