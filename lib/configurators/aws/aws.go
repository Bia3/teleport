...
func policiesTarget(flags configurators.BootstrapFlags, accountID string, partitionID string, identity awslib.Identity, iamClient iamiface.IAMAPI) (awslib.Identity, error) {
	...
	if identity.GetType() == "assumed-role" {
...
// getRoleARNForAssumedRole attempts to resolve assumed-role credentials to the underlying role ARN using IAM API.
// This is necessary since the assumed-role ARN does not include the role path, so we cannot reliably reconstruct
// the role ARN ourselves. When assumed-role credentials are used, we need the role ARN for
