...
func buildSTSAssumeRoleStatements(flags configurators.BootstrapFlags, targetCfg targetConfig, boundary bool) ([]*awslib.Statement, error) {
	if boundary {
		// for boundary policy, we need to add sts permission if any assume role is configured.
		if !flags.ForceBoundaryAssumeRoles && targetCfg.assumeAWSRoles.len() == 0 {
			return nil, nil
		}
		return []*awslib.Statement{{
			Effect:    awslib.EffectAllow,
	...
	// for permissions policy, we only need to add permissions for external roles.
	if len(targetCfg.assumeAWSRoles.external) == 0 {
		return nil, nil
	}
	return []*awslib.Statement{{
		Effect:    awslib.EffectAllow,
		Actions:   stsActions,
		Resources: targetCfg.assumeAWSRoles.external,
	}}, nil
}
...
// awsAssumeRoleARNs is a helper struct organizing AWS IAM roles configured for
// matchers/databases as "internal" roles and "external" roles.
// A role is considered "external" if external_id is set to a non-empty string.
type awsAssumeRoleARNs struct {
	// internal are the AWS role ARNs that have no external_id configured.
	internal []string
	// external are the AWS role ARNs that have an external_id configured.
	external []string
}

// len is a helper function that returns the total number of internal and external
// AWS roles.
func (a awsAssumeRoleARNs) len() int {
	return len(a.internal) + len(a.external)
}
...
type targetConfig struct {
	...
	// assumeAWSRoles are the AWS IAM roles that the target identity needs to
	// be able to assume.
	assumeAWSRoles awsAssumeRoleARNs
}
...
func getTargetConfig(flags configurators.BootstrapFlags, cfg *servicecfg.Config, target awslib.Identity) (targetConfig, error) {
	out := targetConfig{identity: target}
	...
	if !targetIsAssumeRole {
		roles, err := getAWSAssumeRoleARNs(flags, awsMatchers, databases)
		if err != nil {
			return targetConfig{}, trace.Wrap(err)
		}
		out.assumeAWSRoles = roles
	}
	out.awsMatchers = matchersForTarget(awsMatchers, target, targetIsAssumeRole)
	out.databases = databasesForTarget(databases, target, targetIsAssumeRole)
	return out, nil
}
...
func isTargetAWSAssumeRole(flags configurators.BootstrapFlags, matchers []services.AWSMatcher, databases []*servicecfg.Database, target awslib.Identity) bool {
	...
	if slices.Contains(flags.ForceExternalAssumeRoles, targetARN) {
		return true
	}
...
// getAWSAssumeRoleARNs returns all internal and external AWS roles from cli flags, AWS matchers, and databases
// that the target identity will need to be able to assume.
func getAWSAssumeRoleARNs(flags configurators.BootstrapFlags, matchers []services.AWSMatcher, databases []*servicecfg.Database) (awsAssumeRoleARNs, error) {
	internal := make(map[string]struct{})
	external := make(map[string]struct{})
	for _, roleARN := range flags.ForceExternalAssumeRoles {
		if roleARN == "" {
			continue
		}
		external[roleARN] = struct{}{}
	}
	for _, matcher := range matchers {
		if matcher.AssumeRole.RoleARN == "" || !supportsAWSAssumeRole(matcher) {
			continue
		}
		if matcher.AssumeRole.ExternalID == "" {
			internal[matcher.AssumeRole.RoleARN] = struct{}{}
		} else {
			external[matcher.AssumeRole.RoleARN] = struct{}{}
		}
	}
	for _, db := range databases {
		...
		if db.AWS.ExternalID == "" {
			internal[db.AWS.AssumeRoleARN] = struct{}{}
		} else {
			external[db.AWS.AssumeRoleARN] = struct{}{}
		}
	}
	return awsAssumeRoleARNs{
		internal: utils.StringsSliceFromSet(internal),
		external: utils.StringsSliceFromSet(external),
	}, nil
}
