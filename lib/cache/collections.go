...
// executor[R, RD] is a specific way to run the collector operations that we need
// for the genericCollector for a generic resource type R and its reader type RD.
type executor[R types.Resource, RD any] interface {
	...
	// getReader returns the appropriate reader type RD based on the health status of the cache.
	// Reader type RD provides getter methods related to the collection, e.g. GetNodes(), GetRoles().
	// Note that cacheOK set to true means that cache is overall healthy and the collection was confirmed as supported.
	getReader(c *Cache, cacheOK bool) RD
}

// genericCollection is a generic collection implementation for resource type R with collection-specific logic
// encapsulated in executor type E. Type RD provides getter methods related to the collection, e.g. GetNodes(),
// GetRoles().
...
func (g *genericCollection[R, _, _]) fetch(ctx context.Context, cacheOK bool) (apply func(ctx context.Context) error, err error) {
...
func (g *genericCollection[R, _, _]) processEvent(ctx context.Context, event types.Event) error {
	switch event.Type {
	case types.OpDelete:
...
func (g *genericCollection[_, _, _]) watchKind() types.WatchKind {
	return g.watch
}
